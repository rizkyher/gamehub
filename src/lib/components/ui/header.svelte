  ...
  <div class="inline-block relative group cursor-default">
    <h1 class="flex justify-center text-6xl md:text-8xl font-bold text-slate-900 mb-6 tracking-wide uppercase z-10 relative">
      {#each letters as letter, i}
        <span class="pixel-wave inline-block hover:text-[#41B78E] transition-colors duration-200" style="animation-delay: {i * 100}ms">
          {@html letter === " " ? "&nbsp;" : letter}
        </span>
      {/each}
    </h1>
    ...
    <div
      class="h-3 w-full bg-[#F9BA72] border-[3px] border-black absolute -bottom-2 left-0
